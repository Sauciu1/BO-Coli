...
class SingleGroup:
    ...
    def write_data_to_manager(self):
        mask = self.bayes_manager.data["group"] != self.group_label
        self.bayes_manager.data = self.bayes_manager.data[mask]
        self.bayes_manager.data = pd.concat(
            [self.bayes_manager.data, self.group_df], ignore_index=True
        )
...
if __name__ == "__main__":
    ...
    for group_label, group_df in st.session_state.bayes_manager.get_groups().items():
        ...
        st.session_state.groups[group_label].render()
