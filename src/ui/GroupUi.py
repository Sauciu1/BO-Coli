...
class GroupUi:
    ...
    def groups(self):
        ...
        removed_groups = set(st.session_state.groups.keys()) - set(
            current_groups.keys()
        )
    ...
    def add_manual_group(self):
        ...
        new_row = {
            **{param: np.nan for param in self.bayes_manager.feature_labels},
            self.bayes_manager.response_label: np.nan,
            self.bayes_manager.group_label: next_group,
            self.bayes_manager.id_label: f"manual_{str(uuid.uuid4())[:8]}",
        }

        self.bayes_manager.data = pd.concat(
            [self.bayes_manager.data, pd.DataFrame([new_row])], ignore_index=True
        )
    ...
    def remove_group(self, group_label: int):
        ...
        self.bayes_manager.data = self.bayes_manager.data[
            self.bayes_manager.data[self.bayes_manager.group_label] != group_label
    ...
    def render_all(self):
        ...
        groups_to_show: dict[str, SingleGroup] = (
            self.pending_groups
        ...
        if st.session_state["show_pending_only"]:
            st.caption(
                f"Showing {len(groups_to_show)} pending of {len(self.groups)} total groups"
            )
        ...
        for group_label, group in groups_to_show.items():
            with st.container():
                ...
                with cols[0]:
                    ...
                    st.write("")
                    if st.button(
                        "🗑️", key=f"delete_group_{group_label}", help="Delete group"
                    ):
    ...
    def _get_new_targets(self):
        ...
        with cols[1]:
            if st.button(
                ...
                disabled=False,
                type="primary",
                help="Generate new point to be investigated using Bayesian Optimisation",
            ):
